...
from backend.config import CAMERAS, PROJECT_ROOT
...
def _load_employee_registry() -> tuple[dict[str, str], list[str]]:
    """
    Load configs/employees.json (name → stable_id mapping).
    Returns (id_to_name dict, ordered name list) for use as fallback.
    """
    cfg = PROJECT_ROOT / "configs" / "employees.json"
    try:
        with open(cfg, encoding="utf-8") as f:
            name_to_id: dict[str, str] = json.load(f)
        id_to_name = {v: k for k, v in name_to_id.items()}
        return id_to_name, list(name_to_id.keys())
    except Exception as exc:
        logger.warning("Could not load employees.json: %s — names will be unavailable", exc)
        return {}, []


# Loaded once at import time; stable IDs map to real employee names
_ID_TO_NAME, _EMPLOYEE_NAMES = _load_employee_registry()
