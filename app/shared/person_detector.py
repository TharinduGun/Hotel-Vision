...
class PersonDetector:
    ...
    # Only pass tracker if config is set AND the file actually exists
        if self.tracker_config:
            from pathlib import Path
            if Path(self.tracker_config).exists():
                kwargs["tracker"] = self.tracker_config
            else:
                print(f"[PersonDetector] tracker_config not found: {self.tracker_config} — using ultralytics default")
